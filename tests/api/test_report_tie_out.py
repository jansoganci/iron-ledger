...
def test_get_report_uses_persisted_control_summary_not_current_entries() -> None:
    ...
    report = Report(
        ...
        company_id=COMPANY_ID,
        period=PERIOD,
        summary="Payroll compared.",
        reconciliations=pack_report_reconciliations([], summary),
    )
    entries = [
        MonthlyEntry(
            id="e1",
    ...
    assert resp.status_code == 200, resp.text
    body = resp.json()
    by_key = {c["key"]: c for c in body["tie_out_summary"]["controls"]}
    assert by_key["payroll"]["status"] == "tied_out"
    assert by_key["supplier_invoices"]["status"] == "source_missing"
